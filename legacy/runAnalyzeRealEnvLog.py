...
#! /usr/bin/env python
#------------------------------------------------------------
# filename: runAnalyzeRealEnvPattern.py
...
REF_PHONENAME_LIST = ['note5','s6','s6edge','note3','g5','g4','grandmax']
refdate = '20170406'
system ('clear')
...
FIG_SAVE_DIR_DATE       = getcwd() + '/fig/realEnvData/' + refdate
FIG_SCATTER_SAVE_DIR    = FIG_SAVE_DIR_DATE + '/scatter/'
FIG_HIST_SAVE_DIR       = FIG_SAVE_DIR_DATE + '/hist/'


intro.intro_RealEnvAnalysis()


print '# [runAnalyzeRealEnvPattern] Configuration'
print '# [runAnalyzeRealEnvPattern] isEDPattern = %s' % isEDPattern
print '# [runAnalyzeRealEnvPattern] isPCSpattern = %s' % isPCSpattern
print '# -------------------------------------------------------------'
...
for i in range(0,len(REF_PHONENAME_LIST)):
    ...
    REF_DATAFRAME_SIGO_CSV_FILENAME = getcwd() + '/csvfiles/realEnvData/' + ref_csvfilename_sigO
    REF_DATAFRAME_SIGX_CSV_FILENAME = getcwd() + '/csvfiles/realEnvData/' + ref_csvfilename_sigX
    ...
    if isPlot:
        ...
        if isEDPattern:
            ...
            matplot.title('Scatter plot '+ ref_devicename + ' RealEnv')
            matplot.xlim([-110,-30])
            matplot.ylim([-20,50])
            matplot.legend(['realEnv sigO','realEnv sigX'])
        ...
        if isBeaconIDhist:
            ...
            matplot.title('Histogram plot ' + ref_devicename + ' RealEnv')
            matplot.xlim([0, 15])
            matplot.grid()
